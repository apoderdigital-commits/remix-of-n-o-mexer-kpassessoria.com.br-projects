-- ============================================================================
-- CRM · Passo 7 — ENVIAR WhatsApp (CRM -> n8n -> Z-API)
-- Quando uma mensagem 'enviada' entra em crm_messages, um gatilho chama o
-- webhook de ENVIO do n8n (net.http_post), passando telefone + texto. O n8n
-- então dispara no Z-API. Não mexe no front nem tem CORS.
...
declare
  ...
  v_conn  record;
begin
  -- só mensagens de texto ENVIADAS pelo CRM
  if new.direcao <> 'enviada' or coalesce(new.tipo, 'texto') <> 'texto' then
    return new;
  end if;

  -- telefone do contato da conversa
  ...
  -- conexão da loja (precisa ter a URL do n8n configurada)
  select * into v_conn
  ...
  -- dispara o webhook do n8n (assíncrono). Se falhar, NÃO quebra o insert.
  begin
