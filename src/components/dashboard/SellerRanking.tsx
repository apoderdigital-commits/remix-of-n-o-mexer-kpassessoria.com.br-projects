  ...
  return (
    <div className="space-y-4">
      ...
      {hasMore && onShowAll && (
        ...
        >
          <Plus className="h-3.5 w-3.5" /> Ver todos os {top10.length} vendedores
        </Button>
      )}
    </div>
  );
    ...
    <>
      ...
      <Dialog open={showAllOpen} onOpenChange={setShowAllOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-base">{icon} {title} — Todos os vendedores</DialogTitle>
          </DialogHeader>
          <FullSellerContent top10={top10} color={color} showChart={false} />
        </DialogContent>
      </Dialog>
    </>
  );
