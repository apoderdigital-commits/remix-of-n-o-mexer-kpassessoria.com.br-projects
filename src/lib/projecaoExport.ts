...
// Cores das três colunas — as mesmas da tela.
const C_ATUAL = "#94a3b8";
const C_DESEJ = "#fbbf24";
const C_PROJ = "#a855f7";
...
function column(x: number, title: string, accent: string, c: ProjCalc): string {
  ...
  return `
    <rect x="${x}" y="${FUNNEL_TOP}" width="${cw}" height="${FUNNEL_H}" rx="16" fill="#13131b" stroke="#26262f"/>
    <rect x="${x}" y="${FUNNEL_TOP}" width="${cw}" height="6" rx="3" fill="${accent}"/>
    <text x="${x + 20}" y="${FUNNEL_TOP + 35}" fill="#ffffff" font-size="20" font-weight="800">${esc(title)}</text>
    ${body}`;
}
...
function sectionFrame(y: number, h: number, title: string, accent: string): string {
  return `
    <rect x="40" y="${y}" width="${W - 80}" height="${h}" rx="16" fill="#13131b" stroke="#26262f"/>
    <rect x="40" y="${y}" width="${W - 80}" height="6" rx="3" fill="${accent}"/>
    <text x="64" y="${y + 40}" fill="#ffffff" font-size="19" font-weight="800">${esc(title)}</text>
    <line x1="56" y1="${y + 58}" x2="${W - 56}" y2="${y + 58}" stroke="#27272f" stroke-width="1"/>`;
}
...
function atualVsDesejado(y: number, a: ProjCalc, d: ProjCalc): string {
  ...
  let body = `
    <text x="${cEtapa}" y="${y + 86}" fill="#9aa3b2" font-size="13">Etapa</text>
    <text x="${cAtual}" y="${y + 86}" fill="#9aa3b2" font-size="13" text-anchor="end">Atual</text>
    <text x="${cDesej}" y="${y + 86}" fill="${C_DESEJ}" font-size="13" text-anchor="end">Desejado</text>
    <text x="${cPct}" y="${y + 86}" fill="#9aa3b2" font-size="13" text-anchor="end">% Atingido</text>`;
  let ry = y + 120;
  rows.forEach(([label, av, dv, money]) => {
    const pct = dv > 0 ? (av / dv) * 100 : null;
    const cor = pct === null ? "#52525b" : pct >= 100 ? "#34d399" : pct >= 80 ? "#fbbf24" : "#f87171";
    const txt = pct === null ? "—" : pct.toFixed(0) + "%";
    const fmt = money ? brl : intf;
    body += `
      <text x="${cEtapa}" y="${ry}" fill="#cbd5e1" font-size="15">${esc(label)}</text>
      <text x="${cAtual}" y="${ry}" fill="#f3f4f6" font-size="16" font-weight="700" text-anchor="end">${esc(fmt(av))}</text>
      <text x="${cDesej}" y="${ry}" fill="${C_DESEJ}" font-size="16" font-weight="700" text-anchor="end">${esc(fmt(dv))}</text>
  ...
  body += `<text x="${W / 2}" y="${boxY - 14}" fill="#9aa3b2" font-size="12" letter-spacing="2" text-anchor="middle">TAXAS DE CONVERSÃO</text>`;
  const bw = 352, gap = 16, startX = 56;
  taxas.forEach(([label, av, dv], i) => {
    const bx = startX + i * (bw + gap);
    body += `
      <rect x="${bx}" y="${boxY}" width="${bw}" height="64" rx="10" fill="#0f0f16" stroke="#27272f"/>
      <text x="${bx + bw / 2}" y="${boxY + 24}" fill="#9aa3b2" font-size="12" text-anchor="middle">${esc(label)}</text>
      <text x="${bx + bw / 2}" y="${boxY + 48}" fill="#f3f4f6" font-size="16" font-weight="700" text-anchor="middle">${esc(pctf(av))}  →  <tspan fill="${C_DESEJ}">${esc(pctf(dv))}</tspan></text>`;
  });

  return sectionFrame(y, AVD_H, "Atual vs Desejado — O quanto batemos da meta?", C_DESEJ) + body;
}
...
function comparativoGeral(y: number, a: ProjCalc, d: ProjCalc, p: ProjCalc): string {
  ...
  let body = `
    <text x="${cEtapa}" y="${y + 86}" fill="#9aa3b2" font-size="13">Etapa</text>
    <text x="${cA}" y="${y + 86}" fill="#9aa3b2" font-size="13" text-anchor="end">Atual</text>
    <text x="${cD}" y="${y + 86}" fill="${C_DESEJ}" font-size="13" text-anchor="end">Desejado</text>
    <text x="${cP}" y="${y + 86}" fill="${C_PROJ}" font-size="13" text-anchor="end">Projetado</text>`;
  let ry = y + 118;
  rows.forEach(([label, fmt]) => {
    body += `
      <text x="${cEtapa}" y="${ry}" fill="#cbd5e1" font-size="15">${esc(label)}</text>
      <text x="${cA}" y="${ry}" fill="#f3f4f6" font-size="16" font-weight="700" text-anchor="end">${esc(fmt(a))}</text>
      <text x="${cD}" y="${ry}" fill="${C_DESEJ}" font-size="16" font-weight="700" text-anchor="end">${esc(fmt(d))}</text>
      <text x="${cP}" y="${ry}" fill="${C_PROJ}" font-size="16" font-weight="700" text-anchor="end">${esc(fmt(p))}</text>`;
    ry += 31;
  });
  return sectionFrame(y, CG_H, "Comparativo Geral — 3 Funis", C_PROJ) + body;
}
...
function planoDeAcao(y: number, notes: string, linhas: string[]): string {
  ...
  let body = `<text x="64" y="${y + 78}" fill="#9aa3b2" font-size="13">Ações e estratégias para o mês projetado</text>`;
  if (!notes.trim()) {
    body += `<text x="64" y="${y + 112}" fill="#52525b" font-size="15">Nenhuma ação registrada.</text>`;
  } else {
    let ly = y + 112;
    linhas.forEach((l) => {
      body += `<text x="64" y="${ly}" fill="#e5e7eb" font-size="15">${esc(l)}</text>`;
      ly += 26;
    });
  }
...
export function buildProjecaoSvg(
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="Arial, Helvetica, sans-serif">
    ...
    ${column(40, "Funil Atual", C_ATUAL, atual)}
    ${column(440, "Funil Desejado", C_DESEJ, desejado)}
    ${column(840, "Funil Projetado", C_PROJ, projetado)}
